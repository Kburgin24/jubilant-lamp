...
local processRoad = require('gameplay/events/customroam/processRoad')
local leaderboardManager = require('gameplay/events/customroam/leaderboardManager')
local activeAssets = require('gameplay/events/customroam/activeAssets')
local checkpointManager = require('gameplay/events/customroam/checkpointManager')
local utils = require('gameplay/events/customroam/utils')
local pits = require('gameplay/events/customroam/pits')
...
local function onBeamNGTrigger(data)
    ...
    if not triggerName:match("^cre_") then
        -- Not a free roam event trigger, ignore
        return
    end

    -- Remove the 'cre_' prefix for processing
        ...
        if event == "enter" and mActiveRace == raceName then
            ...
            if mCurrentFork then
                ...
                elseif checkpointIndex == mCurrentFork.alt and isAlt then
                    ...
                    -- CALL ASSETS DISPLAY HERE, ONE TIME ONLY
                    Assets:displayAssets(data, true)
                end
            ...
            if isValidHit then
                ...
                if nextFork then
                    ...
                    ui_message(raceData.altRoute.altInfo, 10)
                else
...
local function loadExtensions()
    ...
    local freeroamPath = "/lua/ge/extensions/gameplay/events/customroam/"
...
local function formatEventPoi(raceName, race)
    local startObj = scenetree.findObject("cre_staging_" .. raceName)
